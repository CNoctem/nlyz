...
//        Game g = new Game(board, gotc);
    ...
    private void createButton() {
        nextB = new JButton(">");
        nextB.addActionListener(a -> {
            if (game == null) return;
            if (game.hasNext()) {
                board.applyHalfMove(game.next());
            } else {
                logPane.setText(game.getEnd());
            }
        });
    }

    public void loadGame(String pgn) {
        game = new Game(board, pgn);
    }

    public static void main(String[] args) {
        String gotc = "1. Nf3 Nf6 2. c4 g6 3. Nc3 Bg7 4. d4 O-O 5. Bf4 d5 " +
